...
class Backend(ABC):
    ...
    def _load_model_and_tokenizer(self, model_path: str, device: str, dtype: torch.dtype) -> None:
        """
    ...
    def _batch_infer(self, input_data: List[str], batch_size: Optional[int], device: str) -> List[str]:
        """
        ...
        for i in range(0, len(input_data), batch_size):
            ...
            batch_results = [self.tokenizer.decode(output, skip_special_tokens=True) for output in batch_outputs]
            results.extend(batch_results)
        return results

    def _stream_infer(self, input_data: str, max_tokens: Optional[int], device: str) -> Iterator[str]:
        """
